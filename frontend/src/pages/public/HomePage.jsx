...
export default function HomePage() {
  ...
  const personnelDonut = useMemo(() => {
    const arr = [
      { name: 'Teaching', value: computed.totalTeaching, color: '#1565C0' },
      { name: 'Non-Teaching', value: computed.totalNonTeaching, color: '#2E7D32' },
      { name: 'Admin', value: computed.totalAdmin, color: '#7C3AED' },
    ].filter(d => d.value > 0);
    return arr;
  }, [computed]);
  ...
  const genderDonut = useMemo(() => [
    { name: 'Male', value: Number(s.male_count) || 0, color: '#1565C0' },
    { name: 'Female', value: Number(s.female_count) || 0, color: '#DB2777' },
  ].filter(d => d.value > 0), [s]);
      ...
      <section id="access" className="section dash-section dash-section-alt" ref={accessRef}>
        <div className="container">
          ...
          <div className={`rate-cards reveal${accessInView ? ' in-view' : ''}`} style={{ transitionDelay: '0.25s' }}>
            ...
            <div className="rate-card">
              <ActivityIcon className="rate-icon" style={{ color: '#DC2626' }} />
              <div className="rate-val" style={{ color: '#DC2626' }}>{Number(s.dropout_rate) > 0 ? `${Number(s.dropout_rate).toFixed(1)}%` : '—'}</div>
              <div className="rate-label">Dropout Rate</div>
            </div>
            <div className="rate-card">
              <GraduationIcon className="rate-icon" style={{ color: '#7C3AED' }} />
              <div className="rate-val" style={{ color: '#7C3AED' }}>{Number(s.transition_completion_rate) > 0 ? `${Number(s.transition_completion_rate).toFixed(1)}%` : '—'}</div>
            ...
            <div className="quality-grid">
              <div className="dash-card quality-card-main">
                ...
                <div className="quality-bars">
                  <ProgressBar label="Overall Proficiency" value={Number(s.overall_proficiency) || 0} color="#1565C0" animate={qualityInView} />
                  <ProgressBar label="English Independent Readers" value={Number(s.english_readers) || 0} color="#2E7D32" animate={qualityInView} />
                  <ProgressBar label="Filipino Independent Readers" value={Number(s.filipino_readers) || 0} color="#7C3AED" animate={qualityInView} />
                  <ProgressBar label="Math Numeracy" value={Number(s.math_numeracy) || 0} color="#D97706" animate={qualityInView} />
                  <ProgressBar label="Science Literacy" value={Number(s.science_literacy) || 0} color="#DB2777" animate={qualityInView} />
                </div>
              </div>
              <div className="quality-gauges">
                {[
                  { label: 'Overall', value: Number(s.overall_proficiency) || 0, color: '#1565C0' },
                  { label: 'English', value: Number(s.english_readers) || 0, color: '#2E7D32' },
                  { label: 'Math', value: Number(s.math_numeracy) || 0, color: '#D97706' },
                ].map((g, i) => (
            ...
            <div className="hr-grid">
              ...
              <div className="hr-kpis">
                <div className="hr-kpi-row">
                  <UserCheckIcon className="hr-kpi-icon" style={{ color: '#1565C0' }} />
                  <div>
                    <div className="hr-kpi-num">{Number(s.teaching_personnel) || 0}</div>
                    <div className="hr-kpi-label">Teaching Personnel</div>
                  </div>
                  <div className="hr-kpi-bar" style={{ '--fill': `${computed.totalPersonnel ? (computed.totalTeaching / computed.totalPersonnel) * 100 : 0}%`, '--color': '#1565C0' }} />
                </div>
                <div className="hr-kpi-row">
                ...
                <div className="hr-kpi-row">
                  <ClipboardIcon className="hr-kpi-icon" style={{ color: '#7C3AED' }} />
                  <div>
                    <div className="hr-kpi-num">{Number(s.administrative_staff) || 0}</div>
                    <div className="hr-kpi-label">Administrative Staff</div>
                  </div>
                  <div className="hr-kpi-bar" style={{ '--fill': `${computed.totalPersonnel ? (computed.totalAdmin / computed.totalPersonnel) * 100 : 0}%`, '--color': '#7C3AED' }} />
                </div>
                <div className="hr-kpi-row hr-kpi-row-vacant">
                  <AlertCircleIcon className="hr-kpi-icon" style={{ color: '#D97706' }} />
                  <div>
        ...
        <div className="container">
          ...
          <div className={`gov-grid reveal${govInView ? ' in-view' : ''}`} style={{ transitionDelay: '0.1s' }}>
            <div className="gov-kpi-card">
              <AwardIcon className="gov-kpi-icon" style={{ color: '#1565C0' }} />
            ...
            <div className="gov-kpi-card gov-kpi-wide">
              <BuildingIcon className="gov-kpi-icon" style={{ color: '#059669' }} />
              <div>
        ...
        <div className="container">
          <div className="cta-split">
            <div className="cta-split-left">
              ...
              <div className="cta-badge"><SparkleIcon className="cta-badge-icon" />Admissions Open</div>
              <h2>Ready to Be Part of AANNHS?</h2>
              <p>Begin your enrollment journey and join a community committed to academic excellence and character formation.</p>
            ...
            <div className="cta-split-right">
              <div className="cta-right-icon-wrap"><UsersIcon className="cta-right-icon" /></div>
              <div className="cta-right-label">Total Enrollment</div>
              <div className="cta-right-num">{Number(s.enrollment_count) > 0 ? Number(s.enrollment_count).toLocaleString() : '—'}</div>
              <div className="cta-right-sub">Learners currently enrolled in AANNHS for SY {s.active_school_year || 'current year'}.</div>
              {s.enrollment_status && <StatusBadge status={s.enrollment_status === 'Open' ? 'Completed' : 'In Progress'} />}
            </div>
          </div>
        </div>
      </section>
