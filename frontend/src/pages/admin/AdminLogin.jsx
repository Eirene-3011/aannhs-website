...
export default function AdminLogin() {
  ...
  useEffect(() => {
    ...
    api.post('/auth/login', { code })
      .then(res => {
        localStorage.setItem('aannhs_admin_token', res.data.token);
        setStatus('success');
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, var(--red-dark) 0%, var(--red-primary) 100%)', padding: 20 }}>
      <div style={{ background: 'white', borderRadius: 'var(--radius-xl)', padding: '48px 40px', maxWidth: 420, width: '100%', textAlign: 'center', boxShadow: 'var(--shadow-xl)' }}>
        <img src="/uploads/logo.png" alt="AANNHS Logo" style={{ width: 80, height: 80, objectFit: 'contain', margin: '0 auto 20px' }} onError={e => e.target.style.display='none'} />
        <h1 style={{ fontSize: '1.3rem', fontWeight: 800, color: 'var(--gray-900)', marginBottom: 8 }}>AANNHS Admin Panel</h1>
        <p style={{ fontSize: '0.85rem', color: 'var(--gray-500)', marginBottom: 32 }}>Andres A. Nocon National High School</p>
