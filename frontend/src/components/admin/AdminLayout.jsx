...
export default function AdminLayout() {
  ...
  const handleLogout = () => {
    localStorage.removeItem('aannhs_admin_token');
    navigate('/admin/login/307802');
  };
    ...
    <div className={`admin-layout${sidebarOpen ? '' : ' sidebar-collapsed'}`}>
      ...
      <aside className="admin-sidebar">
        <div className="admin-sidebar-header">
          <img src="/uploads/logo.png" alt="AANNHS" className="admin-sidebar-logo" onError={e => e.target.style.display='none'} />
          {sidebarOpen && (
            <div>
              <p className="admin-sidebar-title">AANNHS Admin</p>
              <p className="admin-sidebar-sub">School ID: 307802</p>
            </div>
          )}
